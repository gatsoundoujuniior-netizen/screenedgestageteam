...
def _charger_stats_couverture():
    ...
    try:
        ...
        from db_utils import query_all
        from opensanctions_local import statut_dump

        _PAYS_COV = ["MA", "DZ", "TN", "LY", "SN", "CI", "ML", "BF", "NE", "TG", "BJ", "GW", "GN"]
        _DB_PATH  = os.path.join(os.path.dirname(os.path.abspath(__file__)), "opensanctions_pep.sqlite")
        _CPT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "verification_checkpoint.json")

        # ── Checkpoint : index "CODE -> {noms lowercase}" ────────────────────
        cpt_par_pays = {}
        if os.path.exists(_CPT_PATH):
            try:
                with open(_CPT_PATH, "r", encoding="utf-8") as _f:
                    for _entry in json.load(_f).get("verifies", []):
                        if ":" in _entry:
                            _c, _n = _entry.split(":", 1)
                            _c = _c.upper()
                            cpt_par_pays.setdefault(_c, set()).add(_n.lower().strip())
            except Exception:
                pass
        ...
        if os.path.exists(_DB_PATH):
            ...
            for _code in _PAYS_COV:
                _cnt = _conn.execute(
                    "SELECT COUNT(DISTINCT id) FROM entites WHERE pays LIKE ?",
                    (f"%{_code.lower()}%",)
                ).fetchone()
        ...
        ds = statut_dump()
        return dump_par_pays, traites_par_pays, inseres, ds
    except Exception as _cov_e:
        return {}, {}, {}, {}

_dump_stats, _traites_cov, _inseres_cov, _ds_info = _charger_stats_couverture()
...
_tot_traites = sum(_traites_cov.values())
_tot_inseres = sum(_inseres_cov.values())
_tot_restant = _tot_dump - _tot_traites

_cov1, _cov2, _cov3, _cov4 = st.columns(4)
with _cov1:
    st.metric("PEP dans le dump", f"{_tot_dump:,}",
              help="Entités PEP africaines indexées dans le dump OpenSanctions local")
with _cov2:
...
_rows_cov = []
for _code in _PAYS_COV:
    _nb_dump    = _dump_stats.get(_code, 0)
    _nb_traites = _traites_cov.get(_code, 0)
    _nb_inseres = _inseres_cov.get(_code, 0)
    _nb_restant = _nb_dump - _nb_traites
    _pct        = round(_nb_traites / _nb_dump * 100) if _nb_dump else 0
    _rows_cov.append({
